...
class Task {
 ...
 private:
	...
	void read_input() {
        ...
        int result = 0;
        // pe diagonala
		auto search = dp_true.find(i * n + j);
		if (search != dp_true.end())
		{
			return dp_true[i * n + j];
		}
		if (i == j) {
			return (operands[i] == 'T');
		} else {
    ...
    int falseValue(int i, int j, string operands, string operators) {
        int result = 0;
		auto search = dp_false.find(i * n + j);

		if (search != dp_false.end())
		{
			return dp_false[i * n + j];
		}
            if (i == j) {
                return (operands[i] == 'F');
            } else {
				...
				// numarul de parantezari pentru care obtin false
