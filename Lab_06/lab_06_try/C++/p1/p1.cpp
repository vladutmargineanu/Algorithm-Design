...
minimax(Nim init, int player, int depth)
{
    ...
    {
        return std::pair<int, Move>(init.eval(player), Move());
    }
    ...
    return std::pair<int, Move>(max, moveMax);
}
...
minimax_abeta(Nim init, int player, int depth, int alfa, int beta)
{
    ...
    for (int i = 0; i < moves.size(); i++)
    {
        Move move = moves[i];
        init.apply_move(move);
        int score = -minimax_abeta(init, player, depth - 1,-beta,-alfa).first;
    ...
    return std::pair<int, Move>(alfa, moveMax);
}
